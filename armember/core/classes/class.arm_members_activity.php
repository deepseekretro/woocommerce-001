...
if (!class_exists('ARM_members_activity'))
{
	class ARM_members_activity
	{
		...
		if (version_compare($arm_newdbversion, '6.9.8', '<') && version_compare($arm_newdbversion,'1.8.1','>')) {
	...
	function generateplugincode() {
        ...
        function arm_crop_image() {
			...
			if (!empty($meta_field_name) && !empty($_SESSION['arm_file_upload_arr'][$meta_field_name]) && ($_SESSION['arm_file_upload_arr'][$meta_field_name]==$file_base_name || (is_array($_SESSION['arm_file_upload_arr'][$meta_field_name]) && in_array($file_base_name,$_SESSION['arm_file_upload_arr'][$meta_field_name])))) //phpcs:ignore
			{

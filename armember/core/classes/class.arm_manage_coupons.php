...
if (!class_exists('ARM_manage_coupons'))
{
    class ARM_manage_coupons
    {
        ...
        function arm_load_coupon_data()
        {
            ...
            if( !empty( $get_coupons ))
            {
                ...
                foreach ($get_coupons as $key => $coupon_val) 
                {
                    ...
                    $grid_data[$ai][] = '<tr class="arm_coupons_tr_'.esc_attr($couponID).' row_'.esc_attr($couponID).'"><td class="center"><input class="chkstanard arm_bulk_select_single" type="checkbox" value="'.esc_attr($couponID).'" name="item-action[]"></td>';

                    $grid_data[$ai][] = '<td>'.$coupon_val->arm_coupon_label.'</td>';

                    $grid_data[$ai][] = '<td><a href="'.esc_attr($edit_link).'">'.stripslashes($coupon_val->arm_coupon_code).'</a></td>';
                    ...
                    $gridActionData .= '<a href="' . esc_url($edit_link) . '"><img src="'.MEMBERSHIP_IMAGES_URL.'/grid_edit.png" onmouseover=\'this.src="'.MEMBERSHIP_IMAGES_URL.'/grid_edit_hover.png";\' class="armhelptip" title="'.esc_attr__('Edit Coupon','ARMember').'" onmouseout=\'this.src="'.MEMBERSHIP_IMAGES_URL.'/grid_edit.png";\' /></a>';
                    $gridActionData .= '<a href="javascript:void(0)" onclick="showConfirmBoxCallback('.$couponID.');"><img src="'.MEMBERSHIP_IMAGES_URL.'/grid_delete.png" class="armhelptip" title="'.esc_attr__('Delete','ARMember').'" onmouseover=\'this.src="'.MEMBERSHIP_IMAGES_URL.'/grid_delete_hover.png";\' onmouseout=\'this.src="'.MEMBERSHIP_IMAGES_URL.'/grid_delete.png";\' /></a>';
                    $gridActionData .= $arm_global_settings->arm_get_confirm_box($couponID, esc_html__("Are you sure you want to delete this coupon?", 'ARMember'), 'arm_coupon_delete_btn');
                    $gridActionData .= '</div>';
        ...
        function arm_coupon_form_html($c_discount,$c_type,$period_type,$sdate_status,$edit_mode,$c_sdate,$c_edate,$c_allow_trial,$c_allowed_uses,$c_label,$c_coupon_on_each_subscriptions,$coupon_status,$c_subs,$c_data, $arm_coupon_type = 1, $arm_paid_posts = array()){
            ...
            $arm_coupon_form_html .='<tr class="form-field form-required coupon_period_options '.esc_attr($period_type_section).'">
            ...
            $arm_coupon_form_html .='<tr class="form-field form-required coupon_type_membership_plan '.$arm_display_membership_plan_class.'">
                                        ...
                                        <td>
                                            <select name="arm_subscription_coupons[]" id="arm_subscription_coupons" class="arm_chosen_selectbox arm_coupons_select_box_sub arm_coupon_input_fields" data-placeholder="'.esc_attr__('Select Plan(s)..', 'ARMember').'" multiple>';
                                                ...
                                                if (!empty($subs_data)) {
                                                    ...
                                                    foreach ($subs_data as $sd) {
                                                        $selected_sub_plan=(in_array($sd['arm_subscription_plan_id'], $c_subs)) ? 'selected="selected"' : "";
                                                            $arm_coupon_form_html .='<option value="' . esc_attr($sd['arm_subscription_plan_id']) . '" ' .$selected_sub_plan. '>' . esc_html(stripslashes($sd['arm_subscription_plan_name'])) . '</option>';
                                                    }
                                                }
                                
            $arm_coupon_form_html .='       </select>
                                            <span class="arm_coupon_blank_field_warning">'.esc_html__('Leave blank for apply coupon to all plan(s)', 'ARMember').'</span>
                                        </td>
                                    </tr>';
            if(!empty($arm_pay_per_post_feature->isPayPerPostFeature)){   
            ...
            $arm_coupon_form_html.= '<tr class="form-field form-required coupon_type_paid_post'.$arm_display_paid_post_class.'">
                                        ...
                                        <td>
                                            ...
                                            <span class="arm_coupon_blank_field_warning">'.esc_html__('Leave blank for apply coupon to all paid post(s)', 'ARMember').'</span>
            ...
            $arm_coupon_form_html .='<tr class="form-field form-required">
                                        ...
                                        <td valign="middle">
                                            ...
                                            <i class="arm_helptip_icon armfa armfa-question-circle" title="'.esc_attr__("Leave blank or '0' for unlimited uses.", 'ARMember').'"></i>
                                        </td>
                                    </tr>';
            $arm_coupon_form_html .='<tr class="form-field form-required">

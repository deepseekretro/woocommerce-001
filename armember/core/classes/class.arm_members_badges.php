			...
			if (isset($_REQUEST['action']) && isset($_REQUEST['badge_id']) && $_REQUEST['badge_id'] != '') {
				...
				$badge_html .= '<div class="armFileUploadWrapper" data-iframe="arm_badges_icon">';
					...
					$badge_html .= '<div class="armFileUploadContainer" style="'.(($display_file) ? 'display:none;': '').'">';
						$badge_html .= '<div class="armFileUpload-icon"></div>'.esc_html__('Upload', 'ARMember');
						$badge_html .= '<input id="arm_badges_icon" class="armFileUpload '.$inputclass.'" name="arm_badges_icon" '.esc_attr($inputType).' accept=".jpg,.jpeg,.png,.gif,.bmp" data-file_size="2" data-file_type="badges"/>';
					$badge_html .= '</div>';
					$badge_html .= '<div class="armFileRemoveContainer" style="'.(($display_file) ? 'display:inline-block;': '').'"><div class="armFileRemove-icon"></div>'.esc_html__('Remove', 'ARMember').'</div>';
            ...
            if (isset($_REQUEST['action']) && isset($_REQUEST['badge_id']) && $_REQUEST['badge_id'] != '') {
                ...
                if ($badgeData) {
                    ...
                    if(is_array($achieve_num))
                    {
                        ...
                        { 
                            ...
                            $arm_achivement_edit_response .= '<input type="text" id="arm_edit_achieve_num" 
                                                                name="arm_achieve_num[]" class="arm_achieve_num arm_min_width_50 arm_width_50 arm_text_align_center arm_padding_left_0" onkeypress="javascript:return isNumber (event);"  value="'.$achieve_num_value.'" data-msg-required="'. esc_attr__("Please enter number", "ARMember").'" >';
                            ...
                            $arm_achivement_edit_response .= '<input type="text" id="arm_require_badges_tootip" name="arm_require_badges_tootip[]" class="arm_achivement_badges_tootip" placeholder="'. esc_attr__('Tooltip Title', 'ARMember').'" value="'.esc_attr($arm_achieve_badges_tooltip_title).'">';
                    ...
                    else
                    {
                        ...
                        $arm_achivement_edit_response .= '</span>';
                        $arm_achivement_edit_response .= '<input type="text" id="arm_edit_achieve_num" 
                                                            name="arm_achieve_num[]" class="arm_achieve_num arm_min_width_50 arm_width_50 arm_text_align_center arm_padding_left_0" onkeypress="javascript:return isNumber (event);"  value="'.esc_attr($achieve_num).'" data-msg-required="'. esc_attr__("Please enter number", "ARMember").'" >';
                        $arm_achivement_edit_response .= '<input type="hidden" id="arm_require_achive_badges_id_'.$arm_achievement_cnt.'" 
                        ...
                        $arm_achivement_edit_response .= '<dl class="arm_selectbox arm_achievement_badge_select arm_badge_select column_level_dd arm_width_190">
                                                            <dt><span><img src="' . esc_attr($arm_badges_require_icon['arm_badges_icon']) . '" alt="" class="arm_badge_icon arm_padding_top_5" style="'.$badge_css.'" /><span class="arm_badge_title"> '.$arm_badges_require_icon['arm_badges_name'].'</span></span><input type="text" style="display:none;" value="" class="arm_autocomplete" /><i class="armfa armfa-caret-down armfa-lg"></i></dt>
                                                            <dd>
                                                                ...
                                                                if(!empty($badges_list))
                                                                {
                                                                    foreach ($badges_list as $badge) 
                                                                    {
                                                                        ...
                                                                        */
                                                                        $arm_achivement_edit_response .= '<li data-value="'.esc_attr($badge->arm_badges_id).'" ><img src="' . esc_attr($badge->arm_badges_icon) . '" alt="" class="arm_badge_icon .arm_padding_top_5" align="middle" style="'.$badge_css.'" /><span class="arm_badge_title">'.$badge->arm_badges_name.'</span></li>';
                        ...
                        $arm_achivement_edit_response .= '<input type="text" id="arm_require_badges_tootip" name="arm_require_badges_tootip[]" placeholder="'. esc_attr__('Tooltip Title', 'ARMember').'" class="arm_achivement_badges_tootip" value="'.$arm_achieve_badges_tooltip_title.'">';
                        $arm_achivement_edit_response .= '<div class="arm_achievement_helptip_icon">
                                                                    <div class="arm_achievement_plus_icon arm_achieve_edit_plus_icon arm_helptip_icon tipso_style " title="'. esc_attr__('Add Achievement', 'ARMember').'"  ></div>
                ...
                if (!empty($user_achievements_detail)) {
                    ...
                    foreach($user_achievements_detail as $user_achieve){
                        ...
                        */
                        ...
                        {
                            $res .= '<span id="arm_edit_admin_badge" class="arm_edit_admin_badge armhelptip" title="'.esc_attr($badge_title).'"><img src="' . esc_attr($user_achieve['badge_icon']) . '" alt="" style="'.$badge_css.'" /></span>';
                            $res .= '</div>';
                        }
                    }
                }
                    ...
                    foreach($user_achievements_detail as $user_achieve){
                        ...
                        $res .= '<div class="arm_badge_edit_res_inner_data user_row_' . esc_attr($rowID) . '">';
                            ...
                            */
                            ...
                            $res .= '</span>';
                            $res .= "<a href='javascript:void(0)' onclick='showConfirmBoxCallback(\"{$rowID}\");'><img src='".MEMBERSHIP_IMAGES_URL."/grid_delete_hover.png' class='armhelptip arm_delete_user_achievements_link' title='".esc_attr__('Delete','ARMember')."' onmouseover=\"this.src='".MEMBERSHIP_IMAGES_URL."/grid_delete_hover.png';\" onmouseout=\"this.src='".MEMBERSHIP_IMAGES_URL."/grid_delete_hover.png';\" /></a>";

                            $res .= $arm_global_settings->arm_get_confirm_box($rowID, esc_html__("Are you sure you want to delete user badge?", 'ARMember').'<input type="hidden" id="data-badge_id" value="'.$achieveID.'"><input type="hidden" id="data-user_id" value="'.esc_attr($user_id).'"> <input type="hidden" id="data-parent_badge_id" value="'.esc_attr($parentBadgeID).'">', 'arm_delete_user_achievements_btn');
				...
				if (!empty($user_achievements_detail)) {
					...
					foreach($user_achievements_detail as $user_achieve){
						...
						$res .= '<input class="user_chk_'.esc_attr($user_id).'_'.esc_attr($parentBadgeID).'_'.esc_attr($achieveID).'" type="checkbox" id="arm_badge_chk_'.esc_attr($rowID).'_'.esc_attr($i).'" style="display:none;">';
                                                $res .= '<span class="arm_badge_edit_res_inner_img user_img_'.esc_attr($user_id).'_'.esc_attr($parentBadgeID).'_'.esc_attr($achieveID).' armhelptip" title="'.esc_attr($user_achieve['badge_title']).'" >';
                                                $res .= '<span class="arm_edit_admin_badge arm_edit_user_admin_badge"><img src="' . strstr($user_achieve['badge_icon'],"//") . '" alt="" style="'.$badge_css.'" class="arm_badge_img" id="'.esc_attr($rowID).'_'.esc_attr($i).'"/></span>';
                                                $res .= '</span>';

...
if (!class_exists('ARM_global_settings')) {
    ...
    class ARM_global_settings {
            ...
            if(!empty($_POST['reset_attempts_users']))//phpcs:ignore
            {
                ...
                if(isset($arm_reset_attempts_users) && !empty($arm_reset_attempts_users)) {
                    ...
                    if(in_array('all', $arm_reset_attempts_users)) {
                        ...
                        $delete = $wpdb->query("TRUNCATE TABLE `$ARMember->tbl_arm_fail_attempts`");//phpcs:ignore --Reason: $ARMember->tbl_arm_fail_attempts is table name
                        $delete = $wpdb->query("TRUNCATE TABLE `$ARMember->tbl_arm_lockdown`");//phpcs:ignore --Reason: $ARMember->tbl_arm_fail_attempts is table name. False
                    } else {
        ...
        function arm_filter_get_avatar($avatar, $id_or_email, $size, $default, $alt = '') {
            ...
            if (!empty($user_avatar)) {
                ...
                if ($this->global_settings['enable_gravatar'] == '0') {
                    $avatar = "<img src='" . MEMBERSHIP_IMAGES_URL . "/avatar_placeholder.png' class='avatar arm_grid_avatar arm-avatar avatar-".esc_attr($size)."' width='".esc_attr($size)."' />";
                } else {
        ...
        function arm_addon_activate_license_form_func($arm_lincense_activate_form)
        {
            ...
            $arm_close_btn_img_url = MEMBERSHIP_IMAGES_URL . '/close-button.png';
            ...
            $arm_lincense_activate_form = '<div id="arfactlicenseform" style="display:none;">
                <div class="arfnewactmodalclose" style="float:right;text-align:right;cursor:pointer;" onclick="javascript:return false;"><img src="'. $arm_close_btn_img_url .'" align="absmiddle" /></div>
        ...
        function arm_addon_activate_button_section_func($arm_addon_btn, $arm_addon_section,$plugin_installer,$arm_addon_config_url='')
        {
            ...
            $loader_img_url = MEMBERSHIP_IMAGES_URL . '/arm_loader.gif';
            ...
            if(empty($plugin_installer))
            {
                if ($setact != 1) {
                    ...
                    $arm_addon_btn = '<div class="arm_feature_button_activate_wrapper '. $arm_social_feature_hidden_btn .'">
                        <a href="javascript:void(0)" class="arm_feature_activate_btn arm_feature_settings_switch" data-feature_val="1" data-feature="'.$arm_addon_section.'">'. esc_html__('Activate','ARMember') .'</a>
                        <span class="arm_addon_loader">
                            <svg class="arm_circular" viewBox="0 0 60 60">
                                <circle class="path" cx="25px" cy="23px" r="18" fill="none" stroke-width="4" stroke-miterlimit="7"></circle>
                            </svg>
                    ...
                    $arm_addon_btn .= '<div class="arm_feature_button_deactivate_wrapper '.  $is_deactivate_btn .'">
                        <a href="javascript:void(0)" class="arm_feature_deactivate_btn arm_feature_settings_switch '.$arm_no_config_class.'" data-feature_val="0" data-feature="'. esc_attr($arm_addon_section).'">'. esc_html__( 'Deactivate', 'ARMember' ).'</a>';
            ...
            if (!empty($user_id) && $user_id != 0) {
                ...
                if (!empty($admin_template_id)) {
                    ...
                    if ($admin_template->arm_template_status == '1') {
                        ...
                        $admin_send_mail = $this->arm_send_message_to_armember_admin_users($to_user, $admin_template->arm_template_content, $admin_template->arm_template_subject);
                    }
                }
            }
        }
        ...
        function arm_get_badges_confirm_box($item_id = 0, $confirmText = '', $btnClass = '', $deleteType = '') {
            ...
            $confirmBox .= "<div class='arm_confirm_box_arrow'></div>";
            $confirmBox .= "<div class='arm_confirm_box_text arm_badge_confirm_box_text_".esc_attr($item_id)."'>".esc_html($confirmText)."</div>";
        ...
        function arm_custom_css_detail() {
            global $wp, $wpdb, $arm_slugs, $current_user, $arm_errors, $ARMember, $arm_subscription_plans, $arm_drip_rules,$arm_capabilities_global;
...
if (!function_exists('arm_custom_css_class_info')) {
    ...
    function arm_custom_css_class_info() {
        $arm_custom_css_info = apply_filters('arm_available_css_info', array(
            ...
            'arm_form' => array(
                'section_title' => array(
                    'title' => esc_html__('Other Forms / Edit Profile', 'ARMember'),

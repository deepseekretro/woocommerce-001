...
if ( is_object( $order ) && ! empty( $resources ) ) {
	...
	?><h2><?php echo esc_html( apply_filters( 'wc_api_manager_email_api_product_heading', __( 'API Product Information', 'woocommerce-api-manager' ) ) ); ?></h2>

	<div style="margin-bottom: 40px;">
		<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;" border="1">
			<thead>
			<tr>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php echo esc_html( apply_filters( 'wc_api_manager_email_product_heading', __( 'Product', 'woocommerce-api-manager' ) ) ); ?></th>
			...
			foreach ( $resources as $resource ) {
				...
				<tr>
					<td class="td" style="text-align:<?php echo esc_attr( $text_align ); ?>; vertical-align:middle; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;">
						<?php echo esc_html( $product_object->get_title() ); ?>
					</td>
					...
					<td class="td" style="text-align:center; vertical-align:middle; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;">
						...
						if ( WCAM()->get_wc_subs_exist() && ! empty( $resource->sub_id ) ) {
							echo esc_html( ( WC_AM_SUBSCRIPTION()->has_end_date_by_sub( $resource->sub_id ) ) ? date_i18n( wc_date_format(), WC_AM_SUBSCRIPTION()->get_subscription_time_by_sub_id( $resource->sub_id, 'end', 'site' ) ) : _x( 'When Cancelled', 'Used as end date for an indefinite subscription', 'woocommerce-api-manager' ) );
						} else {
				...
				<?php if ( ! $hide_product_order_api_keys ) { ?>
					<tr>
						<td class="td" colspan="4" style="text-align:<?php echo esc_attr( $text_align ); ?>; vertical-align:middle; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;">
							<strong><?php echo esc_html( apply_filters( 'wc_api_manager_email_product_order_api_keys_row', __( 'Product Order API Key(s):', 'woocommerce-api-manager' ) ) ); ?></strong>
 ...
 ?>
			...
			if ( ! $hide_master_api_key ) {
				?>
				<tr>
					<th class="td" scope="row" colspan="4" style="text-align:center;"><?php echo esc_html( apply_filters( 'wc_api_manager_email_master_api_key_row', __( 'Master API Key', 'woocommerce-api-manager' ) ) ); ?></th>
				</tr>
				<tr>
					<td class="td" scope="row" colspan="4" style="text-align:center;"><?php echo esc_attr( WC_AM_USER()->get_master_api_key( $order->get_customer_id() ) ); ?></td>
				</tr>
				<tr id="email_master_api_key_message_row">
					<td class="td" scope="row" colspan="4"
						style="text-align:center;"><?php echo esc_html( apply_filters( 'wc_api_manager_email_master_api_key_message_row', __( 'A Master API Key can be used to activate any and all products.', 'woocommerce-api-manager' ) ) ); ?></td>
				</tr>
				<?php
			}

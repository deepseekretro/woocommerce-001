	...
	if ( $master_api_key_status ) {
		...
		if ( $resources ) {
			...
			if ( ! $hide_master_api_key ) {
				...
				<table class="woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_api_manager my_account_orders">
					...
					<tr id="master-api-key-header">
						<th class="master-api-key"><span class="nobr"><?php echo esc_html( apply_filters( 'wc_api_manager_my_account_master_api_key_heading', __( 'Master API Key - Can be used to activate any product.', 'woocommerce-api-manager' ) ) ); ?></span>
						</th>
					</tr>
					</thead>
					<tbody>
			...
			<table class="woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_api_manager my_account_orders">
				...
				<tr id="product-order-api-key-header">
					...
					<?php if ( ! $hide_product_order_api_keys ) { ?>
						<td>
							<?php echo esc_html( apply_filters( 'wc_api_manager_my_account_product_order_api_key_message', __( 'A Product Order API Key is used to activate a single product from a single order.', 'woocommerce-api-manager' ) ) ); ?>
						</td>
					<?php } ?>
				</tr>
				</tbody>
			...
			<table class="woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_api_manager my_account_orders">
				<thead>
				<tr>
					<th class="<?php echo esc_attr( 'api-manager-software-product' ); ?>"><span class="nobr"><?php echo esc_html( apply_filters( 'wc_api_manager_my_account_product_title_heading', __( 'Product Title', 'woocommerce-api-manager' ) ) ); ?></span></th>
					...
					if ( ! $hide_product_order_api_keys ) {
						?>
						<th class="<?php echo esc_attr( 'api-manager-key' ); ?>"><span class="nobr" style="white-space: nowrap;"><?php echo esc_html( apply_filters( 'wc_api_manager_my_account_product_order_api_key_heading', __( 'Product Order API Key', 'woocommerce-api-manager' ) ) ); ?></span></th>
					<?php } ?>
				...
				foreach ( $resources as $resource ) {
					...
					if ( $is_api && ( $order_completed_status || ( WCAM()->get_grant_access_after_payment() && $order_processing_status ) ) ) {
						...
						$skip_key_duplicates = is_array( $product_ids ) && in_array( $product_id, $product_ids, true ) && $total_product_ids[ $product_id ] > 1;
						...
						if ( $resource->sub_id != 0 ) {
							...
							$next_payment = ( WC_AM_SUBSCRIPTION()->has_next_payment_by_sub( $resource->sub_id ) ) ? esc_html( date_i18n( wc_date_format(), WC_AM_SUBSCRIPTION()->get_subscription_time_by_sub_id( $resource->sub_id, 'next_payment', 'site' ) ) ) : esc_html__( 'Pending Cancellation on ', 'woocommerce-api-manager' ) . '<br>' . esc_html( $end_date );
						} elseif ( $resource->access_expires > 0 ) {
							$next_payment = $is_expired && ! $grace_period_expired ? esc_html__( 'Renewable until: ', 'woocommerce-api-manager' ) . esc_html( WC_AM_FORMAT()->unix_timestamp_to_date( WC_AM_GRACE_PERIOD()->get_expiration( $resource->api_resource_id ), true ) ) : esc_html( WC_AM_FORMAT()->unix_timestamp_to_date( $resource->access_expires, true ) );
						} else {
							...
							if ( WCAM()->get_wc_subs_exist() && ! empty( $resource->sub_id ) ) {
								...
								<tr class="order">
									...
									<td class="api-manager-activations">
										<?php
										if ( ! $hide_product_order_api_keys ) {
											echo esc_attr( $resource->activations_total ) . esc_html__( ' out of ', 'woocommerce-api-manager' ) . absint( $resource->activations_purchased_total );
										} else {
											echo esc_attr( $total_activations ) . esc_html__( ' out of ', 'woocommerce-api-manager' ) . absint( $total_activations_purchased );
										}
									...
									<td class="api-manager-expire" style="white-space: nowrap;">
										<?php
										echo $next_payment; // phpcs:ignore
										?>
										<hr>
										...
										<td class="api-manager-license-key">
											...
											if ( $is_expired && ! $grace_period_expired ) {
												...
												if ( ! empty( $discount ) ) {
													?>
													<hr><?php
													/* translators: Placeholder: %s - Percentage discount amount */
													echo esc_html( sprintf( __( 'At a %s discount.', 'woocommerce-api-manager' ), $discount . '%' ) );
												}
											} else {
									...
									<td class="api-manager-expire" style="white-space: nowrap;">
										...
										<p class="order-again">
											<a href="<?php echo esc_url( WC_AM_URL()->api_resource_renewal_url_my_account( $resource->api_resource_id, $product_id, $item_quantity ) ); ?>"
												class="button"><?php
												esc_html_e( 'Renew', 'woocommerce-api-manager' ) . '</a></p>';
											...
											<td colspan="3" style="border-right: 0; padding-left: 5em;">
												...
												if ( filter_var( $activation_info->object, FILTER_VALIDATE_URL ) ) {
													...
													<a style="text-align:left; vertical-align: middle; border-left: 0; padding-left: 1.5em;"
														...
														target="_blank"><?php echo esc_html( WC_AM_URL()->remove_url_prefix( $object ) ); ?></a><span
										...
										<tr class="api-manager-domains">
											<td colspan="4" style="border-right: 0; padding-left: 5em;">
												...
												if ( filter_var( $activation_info->object, FILTER_VALIDATE_URL ) ) {
													...
													$object     = ! empty( $activation_info->object ) && substr( $activation_info->object, $obj_length - 1, $obj_length ) === '/' ? substr( $activation_info->object, 0, $obj_length - 1 ) : $activation_info->object;
													?>
													<a style="text-align:left; vertical-align: middle; border-left: 0; padding-left: 1.5em;" href="<?php echo esc_url( $activation_info->object ); ?>" target="_blank"><?php echo esc_html( WC_AM_URL()->remove_url_prefix( $object ) ); ?></a><span style="vertical-align: middle; padding-left: 1.5em;"><?php echo esc_html__( ' Activated on ', 'woocommerce-api-manager' ) . esc_html( WC_AM_FORMAT()->unix_timestamp_to_date( $activation_info->activation_time ) ); ?></span><?php
												} else {
													?>
													<span style="vertical-align: middle;"><?php echo esc_html( $activation_info->object ) . esc_html__( ' Activated on ', 'woocommerce-api-manager' ) . esc_html( WC_AM_FORMAT()->unix_timestamp_to_date( $activation_info->activation_time ) ); ?></span><?php
												}
			...
			<div class="woocommerce-Message woocommerce-Message--info woocommerce-info">
				<a class="woocommerce-Button button" href="<?php echo esc_url( apply_filters( 'wc_api_manager_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
		...
		<?php
	} // end if master API key not disabled

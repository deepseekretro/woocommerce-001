...
if ( ! empty( $user_id ) ) {
	...
	if ( $master_api_key_status ) {
		...
		if ( $resources ) {
			...
			$columns         = apply_filters( 'wc_api_manager_download_columns', [
 ...
 ?>
				...
				foreach ( $resources as $resource ) {
					...
					if ( $is_api && $is_downloadable && ( $order_completed_status || ( WCAM()->get_grant_access_after_payment() && $order_processing_status ) ) ) {
						...
						/**
						 * Skip duplicates based on Product ID and Order ID.
						 */
						$purchase_key = $product_id . '_' . $order_id;

						if ( isset( $unique_purchases[ $purchase_key ] ) ) {
							continue; // skip duplicate
						} else {
							$unique_purchases[ $purchase_key ] = true;
						}

						// legacy code from 3.3.2 for reference that was in place before the above code
						//
...
if ( ! empty( $dropbox_app_key ) ) :
	?><script type="text/javascript" src="https://www.dropbox.com/static/api/2/dropins.js" id="dropboxjs" data-app-key="<?php echo esc_attr( $dropbox_app_key );

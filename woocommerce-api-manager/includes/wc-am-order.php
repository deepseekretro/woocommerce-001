...
class WC_AM_Order {
	...
	private function __construct() {
		...
		if ( WCAM()->get_grant_access_after_payment() ) {
			 ...
			 * @since 2.0.20
			 */
			add_action( 'woocommerce_order_status_processing', [ $this, 'update_order' ] );
		}

		add_action( 'woocommerce_order_status_completed', [ $this, 'update_order' ] );
		add_action( 'woocommerce_order_status_changed', [ $this, 'remove_order' ], 10, 3 );
		add_action( 'woocommerce_order_partially_refunded', [ $this, 'order_partially_refunded' ], 10, 2 );
		add_action( 'woocommerce_order_fully_refunded', [ $this, 'order_fully_refunded' ], 10, 2 );
		add_action( 'woocommerce_refund_deleted', [ $this, 'refund_deleted' ], 10, 2 );
		add_action( 'woocommerce_delete_order_items', [ $this, 'delete_order' ] );
		add_action( 'woocommerce_before_delete_order', [ $this, 'before_delete_order' ] );
		add_action( 'woocommerce_before_trash_order', [ $this, 'before_delete_order' ] );
		add_action( 'woocommerce_delete_order', [ $this, 'delete_order' ] );
		add_action( 'woocommerce_trash_order', [ $this, 'delete_order' ] );
		add_action( 'woocommerce_before_delete_order_item', [ $this, 'delete_order_item' ] );
		add_action( 'wp_trash_post', [ $this, 'trash_order' ] );
		add_action( 'untrashed_post', [ $this, 'untrashed_order' ] );
		add_action( 'edit_post', [ $this, 'edit_order' ], 10, 2 );
		add_action( 'woocommerce_email_before_order_table', [ $this, 'email_api_keys' ], 10, 3 );
		add_action( 'woocommerce_order_details_before_order_table', [ $this, 'order_details_before_order_table' ], 10, 1 );
	}
	...
	public function update_api_order( $order_id ) {
		...
		if ( ! empty( $existng_products ) ) {
			...
			$resources = $wpdb->get_col( $wpdb->prepare( $sql, $order_id ) ); // phpcs:ignore
	...
	public function remove_order( $order_id, $old_status, $new_status ) {
		...
		$order_statuses = WCAM()->get_grant_access_after_payment() ? [
			'completed',
			'processing',
		] : [
			'completed',
	...
	public function order_details_before_order_table( $order ) {
		...
		if ( ! empty( $resources ) && WC_AM_ORDER_DATA_STORE()->has_api_product( $order ) ) {
			wc_get_template( 'myaccount/related-api-resources.php', [
				'order'     => $order,
				'resources' => $resources,
			], '', WCAM()->plugin_path() . '/templates/' );
		}
				...
				if ( ! WC_AM_FORMAT()->empty( $current_api_resource ) ) {
					...
					if ( $is_renewed_api_resource && $api_resource_renewal_not_updated ) {
						...
						if ( ! WC_AM_FORMAT()->empty( $previous_api_resource_object ) && $current_api_resource->product_id === $previous_api_resource_object->product_id ) {
							...
							/**
							 * @NOTE For some reason the previous developer was only executing the following code if there were any previous activation IDs.
							 * But if there weren't, then it would generate additional API keys because it wouldn't delete the existing resources, and you'd end up with duplicate product keys.
							 * Commenting out the condition below will bump the expiration period as set by the product details.
							 */
							...
							/**
							 * @NOTE See note at the beginning of the commented out if statement.
							 */
							//}

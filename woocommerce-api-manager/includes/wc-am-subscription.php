...
class WC_AM_Subscription {
	...
	private function __construct() {
		if ( WCAM()->get_wc_subs_exist() ) {
			 ...
			 * @since 1.5
			 */
			add_action( 'woocommerce_subscription_status_cancelled', [ $this, 'delete_subscription' ] );
			add_action( 'woocommerce_subscription_status_expired', [ $this, 'delete_subscription' ] );
			add_action( 'subscriptions_cancelled_for_order', [ $this, 'delete_subscription' ] );
			add_action( 'subscriptions_expired_for_order', [ $this, 'delete_subscription' ] );
			add_action( 'woocommerce_subscription_trashed', [ $this, 'delete_subscription' ] );
			add_action( 'woocommerce_subscription_deleted', [ $this, 'delete_subscription' ] );
			add_action( 'woocommerce_api_delete_subscription', [ $this, 'delete_subscription' ] );
			add_action( 'woocommerce_subscription_item_switched', [ $this, 'update_order' ], 10, 4 );
			add_action( 'woocommerce_subscription_status_changed', [ $this, 'refresh_cache' ], 10, 1 );
			add_action( 'woocommerce_subscription_status_changed', [ $this, 'active_to_on_hold' ], 10, 3 );
			add_action( 'woocommerce_subscription_status_changed', [ $this, 'on_hold_to_active' ], 10, 3 );
		}
	}
	 ...
	 * @return int
	 */
	public function get_sub_parent_order_id_for_related_order( $order_id, $order_type = [ 'parent', 'renewal' ] ) {
		...
		if ( is_object( $order ) ) {
			...
			if ( function_exists( 'wcs_get_subscriptions_for_order' ) ) {
				$subscriptions = wcs_get_subscriptions_for_order( $order->get_id(), [ 'order_type' => $order_type ] );
	...
	public function get_last_swtiched_subscription_order_id( $subscription ) {
		...
		if ( is_object( $sub ) ) {
			$last_switch_order = $sub->get_last_order( 'all', [ 'switch' ] );
	 ...
	 * Get the subscription expiration timestamp.
	 *
	 * @see WC_AM_Subscription::get_time()
	 *
	 * @since 2.6
	 *
	 * @param int $sub_id
	...
	public function get_related_order_ids( $order_id, $order_type = 'any' ) {
		...
		if ( class_exists( 'WCS_Related_Order_Store' ) ) {
			...
			if ( in_array( $order_type, [ 'any', 'parent' ] ) && $parent_id ) {
	...
	public function is_wc_subscription( $product_id ) {
		...
		if ( is_object( $product ) && $product->is_type( [
			'subscription',
			'simple-subscription',
	...
	public function is_subscription_cancelled_status( $subscription ) {
		...
		return is_object( $subscription ) && $subscription->has_status( [
			'cancelled',
			'expired',
			'trash',
			'switched',
	...
	public function delete_subscription( $subscription ) {
		...
		if ( is_object( $subscription ) ) {
			...
			if ( ! empty( $sub_id ) ) {
				...
				/**
				 * Fired before the API resources for a WC Subscription are deleted.
				 *
				 * @since 3.3.0
				 *
				 ...
				 * @param int $sub_id
				 */
				do_action( 'wc_api_manager_before_subscription_delete', $subscription, $sub_id );

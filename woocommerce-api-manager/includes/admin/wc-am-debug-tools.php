...
final class WC_AM_Debug_Tools {
	...
	public function add_debug_tools( $tools ) {
		...
		$tools['wc_am_queue_api_resources_repair'] = [
			...
			'callback' => [ $this, 'queue_repair_event' ],
		];

		$tools['wc_am_queue_api_resources_cleanup'] = [
			...
			'callback' => [ $this, 'queue_cleanup_event' ],
		];

		$import_has_run = get_option( 'wc_software_add_on_data_added' ) == 'yes';

		$tools['wc_am_queue_wc_software_add_on_data_import'] = [
			...
			'desc'     => sprintf( __( '%1$s%2$s%3$s%4$sNote:%5$s This tool automatically imports the WooCommerce Software Add-On License Keys and Activations, and builds Order line items into API Resources. %6$sThe import event will run only once.%7$s%8$sNote:%9$s For each Product that should be an API Product, select the Product edit > API checkbox, set the Activation Limit on the Product edit > API form, and set the API Access Expires value if desired. All customer API Resources will be udpated with the new values.%10$sNote:%11$s Either add the %12$sWooCommerce API Manager PHP Library for Plugins and Themes%13$s to your plugin or theme, or build a new client to connect to the API Manager APIs according to the %14$sdocumentation%15$s to take advantage of all the features available.', 'woocommerce-api-manager' ), '<strong class="red">', ( $import_has_run ) ? __( 'Import has already run.', 'woocommerce-api-manager' ) : '', '</strong><br>', '<strong class="red">', '</strong>', '<strong class="red">', '</strong>', '<br><strong class="red">', '</strong>', '<br><strong class="red">', '</strong>', '<a href="' . esc_url( 'https://kestrelwp.com/product/woocommerce-api-manager-php-library-for-plugins-and-themes/' ) . '" target="blank">', '</a>', '<a href="' . esc_url( 'https://woocommerce.com/document/woocommerce-api-manager/' ) . '" target="blank">', '</a>' ),
			'callback' => [ $this, 'queue_wc_software_add_on_data_import' ],
		];

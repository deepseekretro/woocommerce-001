...
class WC_AM_Admin_Notices {
	...
	private function __construct() {
		$this->notices = get_option( 'wc_am_admin_notices', [] );
	...
	public function add_notice( $name ) {
		$this->notices = array_unique( array_merge( $this->get_notices(), [ $name ] ) );
	}
	...
	public function remove_notice( $name ) {
		$this->notices = array_diff( $this->get_notices(), [ $name ] );
	...
	public function add_notices() {
		if ( current_user_can( 'manage_woocommerce' ) ) {
			...
			wp_enqueue_style( 'woocommerce-api-manager--activation', plugins_url( '/includes/assets/css/activation.css', WCAM()->get_plugin_file() ), [], WC_AM_VERSION );
			...
			foreach ( $notices as $notice ) {
				if ( ! empty( $this->core_notices[ $notice ] ) && apply_filters( 'wc_api_manager_show_admin_notice', true, $notice ) ) {
					add_action( 'admin_notices', [ $this, $this->core_notices[ $notice ] ] );
				} else {
					add_action( 'admin_notices', [ $this, 'output_custom_notices' ] );
				}
			}
		}
	}

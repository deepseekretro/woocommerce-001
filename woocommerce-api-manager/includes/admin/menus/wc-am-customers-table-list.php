...
class WC_AM_Customers_Table_List extends WP_List_Table {
		...
		if ( ! WC_AM_FORMAT()->empty( $item ) && ! WC_AM_FORMAT()->empty( $item ) ) {
			...
			switch ( $column_name ) {
				case 'customer_name':
					if ( is_object( $order ) ) {
						if ( $customer instanceof WC_Customer ) {
							return sprintf( esc_html__( '%1$s%2$s%3$s', 'woocommerce-api-manager' ), '<a href="' . esc_url( admin_url( 'user-edit.php?user_id=' . absint( $item->user_id ) ) ) . '" target="_blank">' . $customer->get_first_name() . ' ' . $customer->get_last_name() . '<span style="font-size: medium; vertical-align: middle;" class="dashicons dashicons-external"></span></a>', '<hr>', $customer->get_email() );
						}
					}
				...
				case 'access_expires':
					...
					} elseif ( WC_AM_ORDER_DATA_STORE()->is_time_expired( $item->access_expires ?? false ) ) {
	...
	public function get_sortable_columns() {
		return [
			...
			'access_expires' => [ 'access_expires', false ],
		];
	}

...
class WC_AM_Product_Admin {
		...
		if ( empty( $this->product_fields ) ) {
			...
			$this->product_fields = array(
				...
				array(
					...
					'placeholder' => esc_html__( 'Vault-Tec', 'woocommerce-api-manager' ),
					'class'       => '',
					'type'        => 'text'
				),
				array(
		...
		if ( is_object( $post ) ) {
			...
			if ( ! in_array( $product_type, array( 'grouped', 'external' ) ) ) {
				...
				<div id="api_data" class="panel woocommerce_options_panel">
					...
					echo str_replace( ' id=', " data-placeholder='" . esc_html__( 'Select a page &hellip;', 'woocommerce-api-manager' ) . "' data-width='50%' class='wc-am-select' id=", wp_dropdown_pages( $desc_args ) ) . '<span class="description">' . esc_html__( 'Documentation link for My Account.', 'woocommerce-api-manager' ) . '</span>'; // phpcs:ignore
					echo '</p>';
					...
					echo str_replace( ' id=', " data-placeholder='" . esc_html__( 'Select a page &hellip;', 'woocommerce-api-manager' ) . "' data-width='50%' class='wc-am-select' id=", wp_dropdown_pages( $desc_args ) ) . '<span class="description">' . esc_html__( 'A description of the software.', 'woocommerce-api-manager' ) . '</span>'; // phpcs:ignore
					echo '</p>';
					...
					echo str_replace( ' id=', " data-placeholder='" . esc_html__( 'Select a page &hellip;', 'woocommerce-api-manager' ) . "' data-width='50%' class='wc-am-select' id=", wp_dropdown_pages( $desc_args ) ) . '<span class="description">' . esc_html__( 'Changes in the software.', 'woocommerce-api-manager' ) . '</span>'; // phpcs:ignore
					echo '</p>';
					...
					echo str_replace( ' id=', " data-placeholder='" . esc_html__( 'Select a page &hellip;', 'woocommerce-api-manager' ) . "' data-width='50%' class='wc-am-select' id=", wp_dropdown_pages( $desc_args ) ) . '<span class="description">' . esc_html__( 'How to install the software.', 'woocommerce-api-manager' ) . '</span>'; // phpcs:ignore
					echo '</p>';
					...
					echo str_replace( ' id=', " data-placeholder='" . esc_html__( 'Select a page &hellip;', 'woocommerce-api-manager' ) . "' data-width='50%' class='wc-am-select' id=", wp_dropdown_pages( $desc_args ) ) . '<span class="description">' . esc_html__( 'Frequently Asked Questions.', 'woocommerce-api-manager' ) . '</span>'; // phpcs:ignore
					echo '</p>';
					...
					echo str_replace( ' id=', " data-placeholder='" . esc_html__( 'Select a page &hellip;', 'woocommerce-api-manager' ) . "' data-width='50%' class='wc-am-select' id=", wp_dropdown_pages( $desc_args ) ) . '<span class="description">' . esc_html__( 'Screenshots of the software.', 'woocommerce-api-manager' ) . '</span>'; // phpcs:ignore
					echo '</p>';
					...
					echo str_replace( ' id=', " data-placeholder='" . esc_html__( 'Select a page &hellip;', 'woocommerce-api-manager' ) . "' data-width='50%' class='wc-am-select' id=", wp_dropdown_pages( $desc_args ) ) . '<span class="description">' . esc_html__( 'Other details or special facts.', 'woocommerce-api-manager' ) . '</span>'; // phpcs:ignore
					echo '</p>';
				...
				<div id="api_override_chkbx<?php esc_attr_e( $loop ); ?>">
					...
					<p class="form-row form-row-last show_if_api_global_data_set_var<?php esc_attr_e( $loop ); ?> api_global_data_set_hide_onload_var<?php esc_attr_e( $loop ); ?>">
						...
						<input type="text" name="_api_author_var[<?php esc_attr_e( $loop ); ?>]"
							   ...
							   placeholder="<?php esc_html_e( 'Vault-Tec', 'woocommerce-api-manager' ); ?>"/>
					</p>
					...
					echo str_replace( ' id=', " data-placeholder='" . esc_html__( 'Select a page &hellip; (Optional)', 'woocommerce-api-manager' ) . "' data-width='100%' class='wc-am-var-select' id=", wp_dropdown_pages( $desc_args ) ); // phpcs:ignore
					echo '</p>';
					...
					echo str_replace( ' id=', " data-placeholder='" . esc_html__( 'Select a page &hellip; (Recommended)', 'woocommerce-api-manager' ) . "' data-width='100%' class='wc-am-var-select' id=", wp_dropdown_pages( $desc_args ) ); // phpcs:ignore
					echo '</p>';
	...
	public function save_product( $post_id ) {
		...
		$product_type = empty( $_POST[ 'product-type' ] ) ? 'simple' : sanitize_title( stripslashes( $_POST[ 'product-type' ] ) ); // phpcs:ignore
		...
		if ( in_array( $product_type, array(
			...
			WC_AM_PRODUCT_DATA_STORE()->update_meta( $post_id, '_access_expires', ! empty( $_POST[ '_access_expires' ] ) ? absint( $_POST[ '_access_expires' ] ) : 0 ); // phpcs:ignore
			WC_AM_PRODUCT_DATA_STORE()->update_meta( $post_id, '_api_upgrade_notice', isset( $_POST[ '_api_upgrade_notice' ] ) ? sanitize_text_field( $_POST[ '_api_upgrade_notice' ] ) : '' ); // phpcs:ignore
			...
			foreach ( $this->product_fields as $field ) {
				if ( is_array( $field ) && $field[ 'id' ] != '_api_resource_title' ) { // Software Title/Resource Title is deprecated and cannot be changed.
					$data = isset( $_POST[ $field[ 'id' ] ] ) ? wc_clean( $_POST[ $field[ 'id' ] ] ) : ''; // phpcs:ignore
		...
		if ( in_array( $product_type, array( 'variable', 'variable-subscription', 'subscription_variation' ) ) ) {
			$this->save_common_variation_fields( $post_id, $_POST, $update_access_expires, $update_product_orders );
		}
	 ...
	 * @since 2.0
	 *
	 * @param int|mixed $post_id product or variation ID
	 * @param array<string, mixed>|mixed $post posted data
	 * @param bool $parent_update_access_expires optional
	 * @param bool $parent_update_product_orders optional
	 * @return void
	 */
	public function save_common_variation_fields( $post_id, $post, bool $parent_update_access_expires = false, bool $parent_update_product_orders = false ) : void {
		if ( is_numeric( $post_id ) && is_array( $post ) ) {
			...
			if ( ! empty( $variable_post_ids ) && is_array( $variable_post_ids ) ) {
				...
				for ( $i = 0; $i <= $max_loop; $i ++ ) {
					...
					if ( $parent_update_product_orders || ( isset( $post[ '_api_activations_var' ] ) && $current_activations != (int) $post[ '_api_activations_var' ][ $i ] ) ) {
						$update_product_orders = true;
					}
					...
					if ( $parent_update_access_expires || ( $expires_value < $expires_post_value || $expires_value > $expires_post_value ) ) {
						$update_access_expires = true;
					}
					...
					if ( $global_override == 'yes' || isset( $post[ '_api_data_is_global_override' ][ $i ] ) ) {
						...
						/**
						 * Filters whether to update the API Resource access_expires value when the product API Access Expires value is set to a value greater than 0.
						 *
						 * @since 3.3.5
						 *
						 * @param bool $update_access_expires
						 * @param int $variation_id
						 */
						if ( apply_filters( 'wc_api_manager_queue_update_api_resource_access_expires_for_product_variation', $update_access_expires, $variation_id ) ) {
							// handling since v2.4

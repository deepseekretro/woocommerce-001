...
class WC_AM_API_Resource_Data_Store {
	...
	public function get_all_api_resources_for_order_id( $order_id ) {
		...
		if ( ! WCAM()->get_db_cache() ) {
			...
			$resources = $wpdb->get_results( $wpdb->prepare( $sql, $order_id ) ); // phpcs:ignore
			...
			if ( $resources_active_resources_trans !== false ) {
				...
				if ( $resources_sql_trans === false ) {
					...
					// Get the API resource order items for this user.
					$resources_sql = $wpdb->get_results( $wpdb->prepare( $sql, $order_id ) ); // phpcs:ignore
	...
	public function get_all_api_resources_for_sub_parent_id( $sub_parent_id ) {
		...
		if ( ! WCAM()->get_db_cache() ) {
			...
			// Get the API resource order items for this user.
			$resources = $wpdb->get_results( $wpdb->prepare( $sql, $sub_parent_id ) ); // phpcs:ignore
			...
			if ( $resources_active_resources_trans !== false ) {
				...
				if ( $resources_sql_trans === false ) {
					...
					// Get the API resource order items for this user.
					$resources_sql = $wpdb->get_results( $wpdb->prepare( $sql, $sub_parent_id ) ); // phpcs:ignore
	...
	public function get_api_resources_for_user_id( $user_id ) {
		...
		if ( ! WCAM()->get_db_cache() ) {
			...
			// Get the API resource order items for this user.
			$resources = $wpdb->get_results( $wpdb->prepare( $sql, $user_id ) ); // phpcs:ignore
			...
			if ( $resources_active_resources_trans !== false ) {
				...
				if ( $resources_sql_trans === false ) {
					...
					// Get the API resource order items for this user.
					$resources_sql = $wpdb->get_results( $wpdb->prepare( $sql, $user_id ) ); // phpcs:ignore
	...
	public function get_api_resources_for_master_api_key( $mak ) {
		...
		if ( ! WCAM()->get_db_cache() ) {
			...
			// Get the API resource order items for this user.
			$resources = $wpdb->get_results( $wpdb->prepare( $sql, $mak ) ); // phpcs:ignore
			...
			if ( $resources_active_resources_trans !== false ) {
				...
				if ( $resources_sql_trans === false ) {
					...
					// Get the API resource order items for this user.
					$resources_sql = $wpdb->get_results( $wpdb->prepare( $sql, $mak ) ); // phpcs:ignore
	...
	public function get_api_resources_for_master_api_key_or_product_order_api_key( $api_key ) {
		...
		if ( ! WCAM()->get_db_cache() ) {
			...
			$resources = $wpdb->get_results( $wpdb->prepare( $sql, $api_key, $api_key ) ); // phpcs:ignore
			...
			if ( $resources_active_resources_trans !== false ) {
				...
				if ( $resources_sql_trans === false ) {
					...
					// Get the API resource order items for this user.
					$resources_sql = $wpdb->get_results( $wpdb->prepare( $sql, $api_key, $api_key ) ); // phpcs:ignore
	...
	public function get_api_resources_for_product_order_api_key( $poak, $product_id ) {
		...
		if ( ! WCAM()->get_db_cache() ) {
			...
			// Get the API resource order items for this user.
			$resources = $wpdb->get_results( $wpdb->prepare( $sql, $poak, $product_id ) ); // phpcs:ignore
			...
			if ( $resources_active_resources_trans !== false ) {
				...
				if ( $resources_sql_trans === false ) {
					...
					// Get the API resource order items for this user.
					$resources_sql = $wpdb->get_results( $wpdb->prepare( $sql, $poak, $product_id ) ); // phpcs:ignore
	...
	public function get_product_ids_by_api_key( $api_key ) {
		if ( $api_key ) {
			...
			// Get the API resource order items for this user.
			$product_ids = $wpdb->get_col( $wpdb->prepare( $sql, $api_key, $api_key ) ); // phpcs:ignore
	 ...
	 * @param int|null   $parent_product_id optional parent product ID
	 * @return array
	 * @throws \Exception
	 */
	public function get_api_resources_for_api_key_by_product_id( $api_key, $product_id, ?int $parent_product_id = null ) {
		global $wpdb;
		...
		if ( $api_key ) {
			...
			if ( is_numeric( $product_id ) ) {
				...
				// Get the API resource order items for this user.
				$resources = $wpdb->get_results( $wpdb->prepare( $sql, $api_key, $api_key, $product_id ) ); // phpcs:ignore

				// Try fetching resources using the associated API Key with the given $product_id.
				if ( WC_AM_FORMAT()->empty( $resources ) ) {
				...
				// Fallback: if still no resources, look up among child variations (since v2.10.0 of the PHP Library this could be a possibility).
				if ( (int) $parent_product_id > 0 && WC_AM_FORMAT()->empty( $resources ) ) {
					...
					if ( ! empty( $variation_ids ) ) {
						...
						$in_clause     = implode( ',', $variation_ids );

						// Try the master / product order API Key lookup on child variations.
						$sql = "
							SELECT *
							FROM {$wpdb->prefix}" . $this->api_resource_table . "
							WHERE ( master_api_key = %s OR product_order_api_key = %s )
						...
						// If still no resources found, try the associated API key lookup on child variations.
						if ( WC_AM_FORMAT()->empty( $resources ) ) {
							...
							$resources = $wpdb->get_results( $wpdb->prepare( $sql, WC_AM_ASSOCIATED_API_KEY_DATA_STORE()->get_api_resource_id_by_associated_api_key( $api_key ) ) ); // phpcs:ignore
						}
					}
				}
				...
				foreach ( $ids as $product_id ) {
					...
					// Get the API resource order items for this user.
					$resources = array_merge( $resources, $wpdb->get_results( $wpdb->prepare( $sql, $args ) ) ); // phpcs:ignore
				}
	 ...
	 * @throws \Exception
	 */
	public function get_active_api_resources( $api_key, $product_id, $parent_product_id = null ) {
		$resources = $this->get_api_resources_for_api_key_by_product_id( $api_key, $product_id, $parent_product_id );
	...
	public function get_active_resources( $resources ) {
		...
		return ! WC_AM_FORMAT()->empty( $active_resources ) ? $active_resources : [];
	}
	 ...
	 * @throws \Exception
	 */
	public function get_total_activations_for_product_by_api_key( $api_key, $product_id, $user_id = 0, ?int $parent_product_id = null ) {
		$total_activations = 0;
		$user_id           = $user_id ?: WC_AM_USER()->get_user_id_by_api_key( $api_key );
		$resources         = $this->get_active_api_resources( $api_key, $product_id, $parent_product_id );
	...
	public function get_user_products( $user_id ) {
		...
		return ! WC_AM_FORMAT()->empty( $products ) ? $products : [];
	}
	...
	public function get_access_api_key_expirations( $resources, $unlimited_activations ) {
		...
		foreach ( $resources as $resource ) {
			if ( $resource->sub_id == 0 ) {
				...
				if ( $resource->access_expires > 0 ) {
					$next_payment = $is_expired && ! $grace_period_expired ? __( 'Renewable until: ', 'woocommerce-api-manager' ) . WC_AM_FORMAT()->unix_timestamp_to_date( WC_AM_GRACE_PERIOD()->get_expiration( $resource->api_resource_id ), true ) : WC_AM_FORMAT()->unix_timestamp_to_date( $resource->access_expires, true );
				} else {
					$next_payment = __( 'Lifetime Subscription', 'woocommerce-api-manager' );
				}

				if ( WC_AM_ORDER_DATA_STORE()->is_time_expired( $resource->access_expires ?? false ) ) {
					$expires = __( 'Expired', 'woocommerce-api-manager' );
				} else {
					$expires = $resource->access_expires == 0 ? _x( 'Never', 'Used as end date for an indefinite subscription', 'woocommerce-api-manager' ) : WC_AM_FORMAT()->unix_timestamp_to_date( $resource->access_expires, true );
				}

				$results['non_wc_subs_resources'][] = [
					'next_payment'                     => $next_payment,
					'friendly_api_key_expiration_date' => $expires,
				...
				$end_date     = WC_AM_SUBSCRIPTION()->has_end_date_by_sub( $resource->sub_id ) ? WC_AM_SUBSCRIPTION()->get_subscription_end_date_to_display( $resource->order_id ) : '';
				$next_payment = ( WC_AM_SUBSCRIPTION()->has_next_payment_by_sub( $resource->sub_id ) ) ? date_i18n( wc_date_format(), WC_AM_SUBSCRIPTION()->get_subscription_time_by_sub_id( $resource->sub_id, 'next_payment', 'site' ) ) : __( 'Pending Cancellation on ', 'woocommerce-api-manager' ) . ' ' . $end_date;

				$results['wc_subs_resources'][] = [
					'next_payment'                     => $next_payment,
	 ...
	 * @return object|false
	 */
	public function get_resources_by_api_resource_id( int $api_resource_id ) {
		global $wpdb;
	...
	public function get_all_order_ids() {
		...
		return ! WC_AM_FORMAT()->empty( $order_ids ) ? $order_ids : [];
	}
	...
	public function get_all_api_resource_ids() {
		...
		return ! WC_AM_FORMAT()->empty( $api_resource_ids ) ? $api_resource_ids : [];
	}
	...
	public function get_all_product_ids_from_api_resource_table() {
		...
		return ! WC_AM_FORMAT()->empty( $product_ids ) ? $product_ids : [];
	}
	 ...
	 * @return int
	 */
	public function get_sub_id_by_api_resource_id( int $api_resource_id ) : int {
		global $wpdb;
	 ...
	 * @return int
	 */
	public function get_access_expires_by_api_resource_id( int $api_resource_id ) : int {
		global $wpdb;
	...
	public function get_api_resource_ids_by_order( $order ) {
		...
		if ( is_object( $order ) ) {
			...
			$api_resource_ids = $wpdb->get_col( $wpdb->prepare( "
				SELECT api_resource_id
				FROM {$wpdb->prefix}" . $this->api_resource_table . '
				WHERE order_id = %d
			', $order->get_id() ) );
	 ...
	 * @return int[]|false
	 */
	public function get_api_resource_ids_by_access_expires_number_of_days_before_expiration( int $number_of_days ) {
		global $wpdb;
	 ...
	 * @since 3.0
	 *
	 * @param int $number_of_days Number of days after expirationn
	 * @return string[]|false
	 */
	public function get_api_resource_ids_by_access_expires_number_of_days_after_expiration( int $number_of_days ) {
		global $wpdb;
	...
	public function delete_api_resource_by_api_resource_id( $api_resource_id ) {
		return $this->delete_by( [ 'api_resource_id' => $api_resource_id ], [ '%d' ] );
	}

...
class WC_AM_Product_Data_Store {
	...
	public function get_parent_id_from_product_id( $product_id ) {
		...
		if ( $parent_id === false ) {
			...
			$parent_id = $wpdb->get_var( $wpdb->prepare( $sql, $product_id, 'product_variation' ) ); // phpcs:ignore
            ...
            ";
				...
				$parent_id = $wpdb->get_var( $wpdb->prepare( $sql, $product_id, 'product', 0 ) ); // phpcs:ignore
	            ...
	            ";
					...
					$parent_id = $wpdb->get_var( $wpdb->prepare( $sql, $product_id, 'product' ) ); // phpcs:ignore
	...
	public function is_parent_product( $product_id ) {
		...
		$variation_id = $wpdb->get_var( $wpdb->prepare( $sql, 'product_variation', $product_id ) ); // phpcs:ignore
		...
		if ( ! WC_AM_FORMAT()->empty( $variation_id ) ) {
			...
			$parent_id = $wpdb->get_var( $wpdb->prepare( $sql, 'product', 0 ) ); // phpcs:ignore
	...
	public function filter_get_item_downloads( $files, $item ) {
		...
		return $this->is_api_product( WC_AM_API_RESOURCE_DATA_STORE()->get_api_resource_parent_id( $product_id ) ) ? [] : $files;
	}

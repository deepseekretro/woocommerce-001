...
class WC_AM_Grace_Period_Data_Store implements WCAM_Grace_Period_Data_Store_Interface {
	...
	public function update( $api_resource_id, $expires ) {
		...
		$result = $wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}$this->grace_period_table (`api_resource_id`, `expires`) VALUES (%d, %d) ON DUPLICATE KEY UPDATE `api_resource_id` = VALUES(`api_resource_id`), `expires` = VALUES(`expires`)", $api_resource_id, $expires ) ); // phpcs:ignore
	 ...
	 * @return bool
	 */
	public function delete( $api_resource_id ) : bool {
		global $wpdb;
	...
	public function is_expired( $api_resource_id ) {
		...
		if ( $this->exists( $api_resource_id ) ) {
			...
			$access_expires = WC_AM_API_RESOURCE_DATA_STORE()->get_access_expires_by_api_resource_id( (int) $api_resource_id );
	 ...
	 * @return bool
	 */
	public function add_wc_subscription_expiration_by_order( $order ) : bool {
	 ...
	 * @return bool
	 */
	public function add_wc_subscription_expiration_by_subscription( $subscription ) : bool {
	 ...
	 * @since 2.6
	 *
	 * @param $api_resource_ids|false $api_resource_ids
	 * @return bool
	 */
	private function add_wc_subscription_expiration( $api_resource_ids ) : bool {

		$result           = false;
		$expiration_dates = [];

		if ( is_array( $api_resource_ids ) ) {

			foreach ( $api_resource_ids as $resource ) {

				$sub_id = WC_AM_API_RESOURCE_DATA_STORE()->get_sub_id_by_api_resource_id( (int) $resource->api_resource_id );
	 ...
	 * @return bool
	 */
	public function add_api_manager_subscription_expiration_by_order( $order ) : bool {
	...
	private function add_api_manager_subscription_expiration( $api_resource_ids ) {
		...
		if ( is_array( $api_resource_ids ) ) {
			...
			foreach ( $api_resource_ids as $resource ) {
				...
				$access_expires = WC_AM_API_RESOURCE_DATA_STORE()->get_access_expires_by_api_resource_id( (int) $resource->api_resource_id );
		...
		} elseif ( is_numeric( $api_resource_ids ) ) {

			$access_expires = WC_AM_API_RESOURCE_DATA_STORE()->get_access_expires_by_api_resource_id( (int) $api_resource_ids );
	 ...
	 * @return bool
	 */
	public function delete_expiration_by_order( $order ) : bool {
	 ...
	 * @since 2.6
	 *
	 * @param int|int[]|object|false $api_resource_ids
	 * @return bool
	 */
	private function delete_expiration( $api_resource_ids ) : bool {

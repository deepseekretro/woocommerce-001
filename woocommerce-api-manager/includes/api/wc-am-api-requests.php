...
class WC_AM_API_Requests {
	...
	private function verify_user() {
		...
		if ( ! $master_api_key_status ) {
			...
			if ( empty( $mak ) ) {
				...
				if ( $this->error_log ) {
					WC_AM_LOG()->api_error_log( PHP_EOL . esc_html__( 'Error in verify_user() method. Error message is "Error code 100. A Master API Key did not exist for this customer, and all customers are required to have a Master API Key, so a Master API Key has been created."', 'woocommerce-api-manager' ) . PHP_EOL . wc_print_r( [ 'User ID' => $this->user_id ], true ) );
				}
	...
	private function required( $required_fields ) {
		...
		if ( $missing ) {
			if ( $this->error_log ) {
				WC_AM_LOG()->api_error_log( PHP_EOL . esc_html__( 'Error in required() method. Error message is "Error code 100. The following required query string data is missing:"', 'woocommerce-api-manager' ) . PHP_EOL . wc_print_r( join( ', ', $missing ), true ) . PHP_EOL . PHP_EOL . esc_html__( 'Details from the HTTP(s) query are:', 'woocommerce-api-manager' ) . PHP_EOL . wc_print_r( array_merge( $this->request, [
	...
	private function get_resources() {
		try {
			$parent_product_id = $this->request['parent_product_id'] ?: null;
			$parent_product_id = is_numeric( $parent_product_id ) ? absint( $parent_product_id ) : null;

			$resources = WC_AM_API_RESOURCE_DATA_STORE()->get_active_api_resources( $this->request['api_key'], $this->request['product_id'], $parent_product_id );
			...
			if ( WC_AM_FORMAT()->empty( $resources ) ) {
				...
				$this->error_response( '100', sprintf( __( 'No API Resources exist. Login to %1$sMy Account%2$s to verify there are activations remaining, and the API Key and Product ID are correct.', 'woocommerce-api-manager' ), '<a href="' . WC_AM_URL()->get_api_keys_url() . '" target="blank">', '</a>' ) );
			}
	...
	private function activate() {
		...
		$this->required( [ 'api_key', 'product_id', 'instance' ] );
		 ...
		 * @since 2.0.18
		 */
		WC_AM_SMART_CACHE()->delete_cache( array_merge( $this->request, [ 'resources' => $this->resources ] ) );
		...
		$top_level_data['message'] = sprintf( __( '%1$s out of %2$s activations remaining', 'woocommerce-api-manager' ), $total_activations_remaining, $total_activations_purchased );
		...
		if ( $this->send_response_data ) {
			...
			try {
				...
				$data['resources'] = WC_AM_API_RESOURCE_DATA_STORE()->get_active_api_resources( $this->request['api_key'], $this->request['product_id'], $parent_product_id );
			} catch ( Exception $exception ) {
				if ( $this->debug_log ) {
	...
	private function deactivate() {
		...
		$data['activations_remaining']       = $total_activations_remaining;
		// Legacy API Manager < 2.0, and API Manager PHP Library <= 1.2.
		$top_level_data['activations_remaining'] = sprintf( __( '%1$s out of %2$s activations remaining', 'woocommerce-api-manager' ), $total_activations_remaining, $total_activations_purchased );
	...
	private function information( $type ) {
		$this->required( [ 'product_id', 'plugin_name' ] );
		...
		if ( ! $product_id ) {
			...
			if ( $type == 'serialize' ) {
				// Legacy API Manager < 2.0, and API Manager PHP Library <= 1.2.
				$this->send_serial_error( 'pluginupdatecheck', [ 'download_revoked' => 'download_revoked' ] );
			} else {
		...
		if ( $is_active ) {
			$this->required( [ 'api_key', 'instance', 'version' ] );
	...
	private function update( $type ) {
		...
		if ( $is_active ) {
			$core_required_keys   = [ 'api_key', 'instance' ];
			$active_required_keys = array_merge( $core_required_keys, [ 'version' ] );
		...
		if ( $is_active ) {
			...
			} elseif ( ! empty( $product_data->sub_order_key ) ) { // Local URL.
	...
	private function product_list() {
		...
		$this->required( [ 'api_key', 'instance' ] );
		...
		try {
			...
			if ( WC_AM_FORMAT()->empty( $resources ) ) {
				...
				$this->error_response( '100', sprintf( __( 'No API Resources exist. Login to %1$sMy Account%2$s to verify there are activations remaining, and the API Key is correct.', 'woocommerce-api-manager' ), '<a href="' . WC_AM_URL()->get_api_keys_url() . '" target="blank">', '</a>' ) );
			}
	 ...
	 */
	function success_response( $top_level_data = [], $data = null, $status_code = null ) {
	 ...
	 */
	private function error_response( $code, $message, $top_level_data = [], $status_code = null ) {
		// Legacy API Manager < 2.0, and API Manager PHP Library < 1.2.

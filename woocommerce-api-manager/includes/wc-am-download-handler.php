...
class WC_AM_Download_Handler {
	...
	private function __construct() {
		...
		add_action( 'woocommerce_download_file_redirect', [ $this, 'download_file_redirect' ], 10, 2 );
		add_action( 'woocommerce_download_file_xsendfile', [ $this, 'download_file_xsendfile' ], 10, 2 );
		add_action( 'woocommerce_download_file_force', [ $this, 'download_file_force' ], 10, 2 );
	}
	...
	public function download_product() {
		...
		$product_id = absint( $_GET['am_download_file'] ); // phpcs:ignore
		$order_id   = wc_clean( $_GET['am_order'] ); // phpcs:ignore
		$product    = WC_AM_PRODUCT_DATA_STORE()->get_product_object( $product_id );
		$remote_url = $_GET['remote_url'] == 'yes'; // phpcs:ignore
		$data_store = WC_Data_Store::load( 'customer-download' );

		if ( ! is_object( $product ) || empty( $_GET['hname'] ) || empty( $_GET['hkey'] ) || empty( $_GET['hexpires'] ) ) { // phpcs:ignore
			$this->download_error( esc_html__( 'Invalid download link.', 'woocommerce-api-manager' ) );
		}

		$hash_name = wc_clean( $_GET['hname'] ); // phpcs:ignore
		$password  = wc_clean( $_GET['hkey'] ); // phpcs:ignore
		$expires   = (int) $_GET['hexpires']; // phpcs:ignore
		...
		if ( $remote_url ) {
			...
			if ( ! $file_path ) {
				...
				if ( $parsed_file_path ) {
					// nosemgrep
					$download_range = $this->get_download_range( @filesize( $parsed_file_path['file_path'] ) ); // phpcs:ignore
	...
	public function download( $file_path, $product_id ) {
		...
		add_action( 'nocache_headers', [ $this, 'ie_nocache_headers_fix' ] );
	 ...
	 */
	private function download_headers( $file_path, $filename, $download_range = [] ) {
	...
	private function download_error( $message, $title = '', $status = 404 ) {
		...
		wp_die( $message, esc_html( $title ), [ 'response' => esc_html( $status ) ] ); // phpcs:ignore
	}

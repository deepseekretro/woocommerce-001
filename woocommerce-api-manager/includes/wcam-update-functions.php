								...
								if ( ! empty( $resource->order_id ) ) {
									...
									if ( is_object( $order ) ) {
										...
										if ( ! empty( $associated_api_key_resource ) && ! empty( $activation['instance'] ) && ! empty( $activation['product_id'] ) ) {
											$resource = WC_AM_API_RESOURCE_DATA_STORE()->get_resources_by_api_resource_id( (int) $associated_api_key_resource->api_resource_id );
...
function wc_am_update_200_data_add_product_id_and_add_api_orders_processed_flag_to_api_products() {
	...
	$product_ids = $wpdb->get_col( $wpdb->prepare( $sql, 'product', 'product_variation' ) ); // phpcs:ignore

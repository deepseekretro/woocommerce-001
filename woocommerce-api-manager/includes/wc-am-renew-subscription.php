...
class WC_AM_Renew_Subscription {
	...
	private function __construct() {
		...
		add_action( 'wp', [ $this, 'renew_api_resource' ] );
		...
		add_filter( 'woocommerce_hidden_order_itemmeta', [ $this, 'hidden_order_item_meta' ], 50 );
	...
	public function renew_api_resource() {
		...
		if ( ! empty( $request['_wpnonce'] ) && ! wp_verify_nonce( wc_clean( $_REQUEST['_wpnonce'] ), 'renew_api_resource' ) && ! empty( $request['renew_api_resource'] ) ) { // phpcs:ignore
		...
		if ( ! empty( $request['wc_am_is_renewed_api_resource'] ) && $request['wc_am_is_renewed_api_resource'] == 'yes' && ! empty( $request['api_resource_id'] ) && ! empty( $request['product_id'] ) && is_user_logged_in() ) {
			...
			$resource = WC_AM_API_RESOURCE_DATA_STORE()->get_resources_by_api_resource_id( (int) $request['api_resource_id'] ?? 0 );
			...
			if ( ! WC_AM_FORMAT()->empty( $resource ) && ! empty( WC_AM_API_RESOURCE_DATA_STORE()->get_active_api_resources( $resource->product_order_api_key, $resource->product_id , $parent_product_id ) ) ) {
				...
				WC()->cart->empty_cart();
				WC()->cart->add_to_cart( $resource->product_id, ! empty( $request['item_quantity'] ) ? $request['item_quantity'] : 1, 0, [], [

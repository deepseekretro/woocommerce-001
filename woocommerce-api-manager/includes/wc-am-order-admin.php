...
class WC_AM_Order_Admin {
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_wc_api_manager_delete_activation', [ $this, 'delete_activation' ] );
		...
		if ( WCAM()->is_custom_order_tables_usage_enabled() ) {
			add_filter( 'manage_edit-woocommerce_page_wc-orders_columns', [ $this, 'render_contains_api_product_column' ] );
			add_action( 'manage_woocommerce_page_wc-orders_posts_custom_column', [ $this, 'render_contains_api_product_column_content' ], 10, 2 );
		} else {
			// Non HPOS.
			add_filter( 'manage_edit-shop_order_columns', [ $this, 'render_contains_api_product_column' ] );
			add_action( 'manage_shop_order_posts_custom_column', [ $this, 'render_contains_api_product_column_content' ], 10, 2 );
		}
	}
	...
	public function add_meta_boxes( $post_type, $post_or_order_object ) {
		...
		if ( is_object( $order ) && $current_screen && $current_screen->id === $order_screen_id && WC_AM_ORDER_DATA_STORE()->has_api_product( $order->get_id() ) ) {
			add_meta_box( 'wc_am_master_api_key', esc_html__( 'Master API Key', 'woocommerce-api-manager' ), [ $this, 'master_api_key_meta_box' ], $order_screen_id, 'normal', 'high' );
			add_meta_box( 'wc_am_api_resource', esc_html__( 'API Resources', 'woocommerce-api-manager' ), [ $this, 'api_resource_meta_box' ], $order_screen_id, 'normal', 'high' );
			add_meta_box( 'wc_am_api_resource_activations', esc_html__( 'API Resource Activations', 'woocommerce-api-manager' ), [ $this, 'api_resource_activation_meta_box' ], $order_screen_id, 'normal', 'high' );
		}
	}
			...
			if ( ! WC_AM_ORDER_DATA_STORE()->has_api_product( $order->get_id() ) ) {
				...
				if ( WCAM()->get_wc_subs_exist() ) {
					...
					if ( ! empty( $sub_parent_id ) && (int) $sub_parent_id == (int) $order->get_id() ) {
						...
						$sub_parent_order_id = WC_AM_SUBSCRIPTION()->get_sub_parent_order_id_for_related_order( $order, [ 'switch' ] );
				...
				if ( ! empty( $resources ) ) {
					...
					*/
					...
					if ( WCAM()->get_wc_subs_exist() ) {
						$sub_parent_order_id = WC_AM_SUBSCRIPTION()->get_sub_parent_order_id_for_related_order( $order, [ 'renewal' ] );
					...
					if ( ! empty( $sub_parent_order_id ) ) {
						if ( $order_screen_id == 'woocommerce_page_wc-orders' ) {
							printf( esc_html__( 'See Parent Order %1$s%2$s', 'woocommerce-api-manager' ), '<a href="' . esc_url( self_admin_url() . 'admin.php?page=wc-orders&action=edit&id=' . $url_id ) . '">', ' #' . esc_attr( $url_id ) . '</a>' );
						} else {
							printf( esc_html__( 'See Parent Order %1$s%2$s', 'woocommerce-api-manager' ), '<a href="' . esc_url( self_admin_url() . 'post.php?action=edit&post=' . $url_id ) . '">', ' #' . esc_attr( $url_id ) . '</a>' );
						}
					} else {
						?>
						<p style="padding:0 8px;"><?php esc_html_e( 'No API resources for this order.', 'woocommerce-api-manager' ); ?></p><?php
	...
	public function delete_activation() {
		...
		if ( is_array( $this_post ) ) {
			...
			WC_AM_SMART_CACHE()->delete_cache( wc_clean( [ 'admin_resources' => $admin_resources ] ), true );

...
class WCAM_Background_Updater extends WP_Background_Process {
	...
	public function dispatch() {
		...
		if ( is_wp_error( $dispatched ) ) {
			$logger->error( sprintf( 'Unable to dispatch WooCommerce API Manager updater: %s', $dispatched->get_error_message() ), [ 'source' => 'wc_am_db_updates' ] );
		}
	}
	...
	protected function task( $callback ) {
		...
		include_once __DIR__ . '/wcam-update-functions.php';

		if ( is_callable( $callback ) ) {
			$logger->info( sprintf( 'Running %s callback', $callback ), [ 'source' => 'wc_am_db_updates' ] );
			call_user_func( $callback );
			$logger->info( sprintf( 'Finished %s callback', $callback ), [ 'source' => 'wc_am_db_updates' ] );
		} else {
			$logger->notice( sprintf( 'Could not find %s callback', $callback ), [ 'source' => 'wc_am_db_updates' ] );
		}
	...
	protected function complete() {
		$logger = wc_get_logger();
		$logger->info( 'Data update complete', [ 'source' => 'wc_am_db_updates' ] );

...
class WC_AM_Install {
	...
	private function __construct() {
		...
		// Installation
		register_activation_hook( WCAM()->get_plugin_file(), [ $this, 'install' ] );
		// Add the Settings | Documentation | Support links on the Plugins administration screen
		add_filter( 'plugin_action_links_' . WCAM()->get_file(), [ $this, 'action_links' ] );
	...
	public function init_background_updater() {
		require_once __DIR__ . '/wcam-background-updater.php';
	...
	private function remove_admin_notices() {
		include_once __DIR__ . '/admin/wc-am-admin-notices.php';
		...
		foreach ( $this->db_updates as $version => $update_callbacks ) {
			if ( version_compare( $current_db_version, $version, '<' ) ) {
				foreach ( $update_callbacks as $update_callback ) {
					$logger->info( sprintf( 'Queuing %s - %s', $version, $update_callback ), [ 'source' => 'wc_am_db_updates' ] );
	...
	private function create_options() {
		...
		if ( get_option( 'woocommerce_api_manager_grace_period' ) === false ) {
			update_option( 'woocommerce_api_manager_grace_period', [
				'number' => 14,
				'unit'   => 'days',
			] );
		}
		...
		if ( get_option( 'woocommerce_api_manager_manual_renewal_period' ) === false ) {
			update_option( 'woocommerce_api_manager_manual_renewal_period', [
				'number' => 2,
				'unit'   => 'months',
			] );
		}

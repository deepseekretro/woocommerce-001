...
class WC_API_Manager_Query extends WC_Query {
	...
	public function __construct() {
		...
		add_filter( 'the_title', [ $this, 'change_endpoint_title' ], 11, 1 );

		if ( ! is_admin() ) {
			add_filter( 'query_vars', [ $this, 'add_query_vars' ], 0 );
			add_action( 'parse_request', [ $this, 'parse_request' ], 0 );
			add_filter( 'woocommerce_get_breadcrumb', [ $this, 'add_breadcrumb' ], 10 );
			add_action( 'pre_get_posts', [ $this, 'pre_get_posts' ], 11 );
			add_filter( 'woocommerce_get_query_vars', [ $this, 'add_wc_am_query_vars' ] );
			...
			add_action( 'woocommerce_account_' . $this->api_keys_endpoint . '_endpoint', [ $this, 'api_keys_endpoint_content' ] );
			add_action( 'woocommerce_account_' . $this->downloads_endpoint . '_endpoint', [ $this, 'downloads_endpoint_content' ] );
		}
		...
		if ( WCAM()->is_woocommerce_pre( '3.4' ) ) {
			add_filter( 'woocommerce_account_settings', [ $this, 'add_endpoint_account_settings' ] );
		} else {
			add_filter( 'woocommerce_get_settings_advanced', [ $this, 'add_endpoint_account_settings' ] );
		}
	}
	...
	public function add_breadcrumb( $bread_crumbs ) {
		foreach ( $this->query_vars as $key => $query_var ) {
			if ( $this->is_query( $query_var ) ) {
				$bread_crumbs[] = [ $this->get_endpoint_title( $key ) ];
			}
		}
	...
	public function api_keys_endpoint_content() {
		wc_get_template( 'myaccount/api-keys.php', [ 'user_id' => get_current_user_id() ], '', WCAM()->plugin_path() . '/templates/' );
	}
	...
	public function downloads_endpoint_content() {
		wc_get_template( 'myaccount/api-downloads.php', [ 'user_id' => get_current_user_id() ], '', WCAM()->plugin_path() . '/templates/' );
	}
	...
	public function get_endpoint_url( $url, $endpoint, $value = '', $permalink = '' ) {
		if ( ! empty( $this->query_vars[ $endpoint ] ) ) {
			remove_filter( 'woocommerce_get_endpoint_url', [ $this, 'get_endpoint_url' ] );
	...
	public function add_endpoint_account_settings( $settings ) {
		...
		$this->insert_setting_after( $settings, 'woocommerce_myaccount_view_order_endpoint', [
			$api_keys_endpoint_setting,
			$api_downloads_endpoint_setting,
		], 'multiple_settings' );

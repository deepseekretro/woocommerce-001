...
class WC_AM_Background_Events {
	...
	/** @var WCAM_Events_Background_Process  */
	...
	private function __construct() {
		...
		if ( get_option( 'woocommerce_api_manager_api_resoure_cleanup_data' ) == 'yes' ) {
			add_action( 'wc_am_weekly_event', [ $this, 'queue_weekly_event' ] );
		}

		add_action( 'wc_am_daily_event', [ $this, 'queue_daily_event' ] );
	}
	...
	public function queue_daily_event() {
		...
		$this->background_process->push_to_queue( [ 'task' => 'sub_expire_notification' ] );
	...
	public function queue_weekly_event() {
		...
		$this->background_process->push_to_queue( [ 'task' => 'cleanup_hash' ] );
		$this->background_process->save()->dispatch();
	...
	public function queue_repair_event() {
		...
		$this->background_process->push_to_queue( [ 'task' => 'wc_am_repair_hash' ] );
		$this->background_process->save()->dispatch();
	...
	public function queue_wc_software_add_on_data_import_event() {
		if ( get_option( 'woocommerce_api_manager_translate_software_add_on_queries' ) == 'yes' && get_option( 'wc_software_add_on_data_added' ) === false ) {
			...
			$this->background_process->push_to_queue( [ 'task' => 'wc_am_add_wc_software_add_on_data_event' ] );
			$this->background_process->save()->dispatch();
	...
	private function queue_subscription_30_day_expiration_notification() {
		...
		if ( is_array( $api_resource_ids ) && ! WC_AM_FORMAT()->empty( $api_resource_ids ) ) {
			foreach ( $api_resource_ids as $api_resource_id ) {
	...
	public function send_subscription_expiration_notification( $api_resource_id ) {
		if ( ! WC_AM_FORMAT()->empty( $api_resource_id ) && ! WC_AM_API_RESOURCE_DATA_STORE()->is_lifetime_subscription( $api_resource_id ) ) {
			WC_AM_EMAILS()->send_subscription_expiration_notification( (int) $api_resource_id );
		}
	}
		...
		if ( is_array( $order_ids ) && ! WC_AM_FORMAT()->empty( $order_ids ) ) {
			foreach ( $order_ids as $key => $order_id ) {
				if ( ! WC_AM_FORMAT()->empty( $order_id ) ) {
					$this->background_process->push_to_queue( [
						'task'                   => 'cleanup_expired_api_resources',
						'order_id_api_resources' => $order_id,
					] );
				}
			}
		}
	...
	private function queue_cleanup_expired_api_activations() {
		...
		if ( is_array( $order_ids ) && ! WC_AM_FORMAT()->empty( $order_ids ) ) {
			foreach ( $order_ids as $key => $order_id ) {
				if ( ! WC_AM_FORMAT()->empty( $order_id ) ) {
					$this->background_process->push_to_queue( [
						'task'                     => 'cleanup_expired_api_activations',
						'order_id_api_activations' => $order_id,
					] );
				}
			}
		}
	...
	private function queue_cleanup_expired_grace_periods() {
		...
		if ( is_array( $api_resource_ids ) && ! WC_AM_FORMAT()->empty( $api_resource_ids ) ) {
			foreach ( $api_resource_ids as $key => $api_resource_id ) {
				if ( ! WC_AM_FORMAT()->empty( $api_resource_id ) ) {
					$this->background_process->push_to_queue( [
						'task'                          => 'cleanup_expired_grace_periods',
						'api_resource_id_grace_periods' => $api_resource_id,
					] );
				}
			}
		}
	...
	public function queue_add_new_api_product_orders_event( $product_id ) {
		...
		$this->background_process->push_to_queue( [ 'task' => 'wc_am_add_new_api_product_orders_event' ] );
		$this->background_process->save()->dispatch();
	}
	...
	public function queue_update_api_resource_activations_for_product_event( $product_id ) {
		...
		$this->background_process->push_to_queue( [ 'task' => 'wc_am_update_api_resource_activations_for_product_event' ] );
		$this->background_process->save()->dispatch();
	}
	...
	public function queue_update_api_resource_access_expires_for_product_event( $product_id ) {
		...
		$this->background_process->push_to_queue( [ 'task' => 'wc_am_update_api_resource_access_expires_for_product_event' ] );
		$this->background_process->save()->dispatch();
	}
	...
	private function queue_repair_missing_api_resources() {
		...
		if ( ! WC_AM_FORMAT()->empty( $am_order_ids ) && ! WC_AM_FORMAT()->empty( $wc_order_ids ) ) {
			...
			if ( ! WC_AM_FORMAT()->empty( $order_ids ) ) {
				foreach ( $order_ids as $order_id ) {
					$this->background_process->push_to_queue( [
						'task'                          => 'repair_missing_api_resources',
						'repair_order_id_api_resources' => $order_id,
					] );
				}
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}woocommerce_software_licenses';" ) ) {
			...
			if ( ! WC_AM_FORMAT()->empty( $key_ids ) ) {
				foreach ( $key_ids as $key_id ) {
					$this->background_process->push_to_queue( [
						'task'                           => 'add_wc_software_add_on_data',
						'wc_software_add_on_data_key_id' => $key_id,
					] );
				}
	...
	public function add_wc_software_add_on_data( $key_id ) {
		...
		if ( ! WC_AM_FORMAT()->empty( $key_id ) ) {
			...
			try {
				...
				$software_license_data = $wpdb->get_row( $wpdb->prepare( $license_sql, $key_id ) ); // phpcs:ignore
				...
				if ( ! WC_AM_FORMAT()->empty( $software_license_data ) ) {
					...
					$software_activation_data = $wpdb->get_row( $wpdb->prepare( $activation_sql, $key_id ) ); // phpcs:ignore
	...
	public function queue_update_api_resource_activations_total_event() {
		...
		$this->background_process->push_to_queue( [ 'task' => 'wc_am_update_api_resource_activations_total_event' ] );
		$this->background_process->save()->dispatch();
	}
	...
	private function queue_update_api_resource_activations_total() {
		...
		if ( is_array( $api_resource_ids ) && ! WC_AM_FORMAT()->empty( $api_resource_ids ) ) {
			foreach ( $api_resource_ids as $key => $api_resource_id ) {
				if ( ! WC_AM_FORMAT()->empty( $api_resource_id ) ) {
					$this->background_process->push_to_queue( [
						'task'                         => 'update_api_resource_activations_total',
						'api_resource_api_resource_id' => $api_resource_id,
					] );
				}
			}
		}

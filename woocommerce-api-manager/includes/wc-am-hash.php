...
class WC_AM_Hash {
	...
	public function password_hash( $user_id, $password = '', $algorithm = '', $options = '' ) {
		...
		if ( function_exists( 'password_hash' ) ) {
			...
			if ( ! empty( $hash ) ) {
				...
				if ( ! empty( $hash_data ) ) {
					return [
						'hname'    => $hash_data['hash_name'],
						'hkey'     => $password,
						'hexpires' => $hash_data['hash_time'],
					];
				} else {
					return false;
				}
	...
	public function cleanup_hash() {
		...
		$result = $wpdb->query( $wpdb->prepare( $sql, WC_AM_ORDER_DATA_STORE()->get_current_time_stamp() ) ); // phpcs:ignore
	...
	public function get_expired_hash_count() {
		...
		$expired_hash_count = $wpdb->get_var( $wpdb->prepare( "
			SELECT COUNT(hash_id)
			FROM {$wpdb->prefix}wc_am_secure_hash
			WHERE hash_time < %d
		", WC_AM_ORDER_DATA_STORE()->get_current_time_stamp() ) );
